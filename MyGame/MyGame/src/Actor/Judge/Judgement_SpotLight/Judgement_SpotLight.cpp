...
//コンストラクタ
Judgement_SpotLight::Judgement_SpotLight(IWorld * world, const std::string & name, const Vector3 & position)
...
void Judgement_SpotLight::onDraw() const{
	...
	for (auto t : targets) {
		if (is_In_Distans(t) == true) {
			DebugDraw::DebugDrawFormatString(400, 500, GetColor(255, 255, 255), "範囲内");
		}
	}
}
