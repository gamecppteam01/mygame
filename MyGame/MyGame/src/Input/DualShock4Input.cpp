...
int DualShock4Input::GetPovTriggerDownAngle() const
{
	...
	if ((pov & 8) != 0)return -1;//失敗
	if ((pov & 7) == 7) {
		if((prevPov & 7) != 7)return 315;//左上
		else return -1;
	}
	if ((pov & 6) == 6) {
		if((prevPov & 6) != 6)return 270;//左
		else return -1;
	}
	if ((pov & 5) == 5) {
		if((prevPov & 5) != 5)return 225;//左下
		else return -1;
	}
	if ((pov & 4) == 4) {
		if((prevPov & 4) != 4)return 180;//下
		else return -1;
	}
	if ((pov & 3) == 3) {
		if((prevPov & 3) != 3)return 135;//右下
		else return -1;
	}
	if ((pov & 2) == 2) {
		if((prevPov & 2) != 2)return 90;//右
		else return -1;
	}
	if ((pov & 1) == 1) {
		if((prevPov & 1) != 1)return 45;//右上
		else return -1;
	}
	if ((prevPov & 8) == 0)return -1;//失敗
	else return 0;//上
}
