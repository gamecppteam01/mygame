...
//対象から後ろ側に離れる距離(z軸移動)
static const float backwardLength = 100.0f;
//カメラの自由補正
static const Vector3 moveVector{ 0.0f,0.0f,0.0f };
static const Vector3 targetVector{ 0.0f,-30.0f,-20.0f };
//対象から上側に離れる距離(y軸移動)
...
void OverLookingCamera::onUpdate(float deltaTime)
{
	...
	if (target_ != nullptr) {
		...
		//ターゲットの位置を対象の位置に設定
		targetPos = position_ + Vector3(0.0f, 0.0f, backwardLength);
		targetPos.y = 0.0f;
		//一応yを固定し直す
	...
	Camera::GetInstance().Target.Set(targetPos+ targetVector);
